...
# olası warning çktısını ignore etmek için:
import warnings
...
# pandas kütüphanesinin read_csv() metodu ile csv dosyası okunur ve bir değişkene atılır.
...
def check_df(dataframe, head=7):
    ...
    print("##################### Shape #####################")
    # shape : verinin satır ve sütun uzunluklarını verir.
    print(dataframe.shape)
    print("##################### Types #####################")
    # dtypes: her bir column tipini öğreniyoruz.
...
def grab_col_names(dataframe, cat_th=10, car_th=20):
    ...
    # list comprehension içinde else olmadığı için if for 'un sağında
    # dtype object ise kategorik olarak tutuyoruz.

    cat_cols = [col for col in dataframe.columns if dataframe[col].dtypes == "O"]

     # num_but_cat : numeric gibi ama kategorik
    num_but_cat = [col for col in dataframe.columns if dataframe[col].nunique() < cat_th and dataframe[col].dtypes != "O"]

    # cat_but_car : kategorik ama kardinalitisi yüksek veriler. kateogrik gibi ama kardinal
...
def cat_summary(dataframe, col_name, plot=False):
    ...
    print("##########################################")
    #false olursa bu if bloğu çalışmıyor.

    # true ise if bloğu çalışıyor.
    if plot:
